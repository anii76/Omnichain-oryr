...
contract Rebalancer is NonblockingLzApp, ReentrancyGuard, Pausable {
    ...
    function _nonblockingLzReceive(uint16, bytes memory, uint64, bytes memory payload) internal override whenNotPaused nonReentrant {
        (address tokenIn, address user, uint256 amountIn, address tokenOut, , bytes memory swapCalldata) =
